...
class LayerNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor):
        x = x.permute(0, 2, 3, 1)  # 将输入张量的形状从 (B, C, H, W) 变为 (B, H, W, C)
        x = nn.functional.layer_norm(x, self.normalized_shape, self.weight, self.bias, self.eps)
        x = x.permute(0, 3, 1, 2)
        return x


class PatchMerging2D(nn.Module):  # 类似下采样层
    ...
    @staticmethod
    def _patch_merging_pad_channel_last(x: torch.Tensor):  # 分块、合并
        H, W, _ = x.shape[-3:]
        if (W % 2 != 0) or (H % 2 != 0):
            x = F.pad(x, (0, 0, 0, W % 2, 0, H % 2))  # 填充为偶数
...
class Permute(nn.Module):
    def __init__(self, *args):  # *args 表示 __init__ 方法可以接受多个位置参数
        super().__init__()
        self.args = args
...
class gMlp(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.fc1(x)
        x, z = x.chunk(2, dim=(1 if self.channel_first else -1))  # 将张量 x 在指定维度 dim 上分成两个部分
...
class SoftmaxSpatial(nn.Softmax):
    def forward(self, x: torch.Tensor):
        if self.dim == -1:
            B, C, H, W = x.shape
            return super().forward(x.view(B, C, -1)).view(B, C, H,
                                                          W)  # (B,C,H,W)  (B,C,H*W)  最后一个维度进行softmax  (B,C,H,W)
        elif self.dim == 1:
            B, H, W, C = x.shape
...
class mamba_init:
    ...
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4):
        ...
        if dt_init == "constant":
            nn.init.constant_(dt_proj.weight, dt_init_std)  # 初始化权重参数
        elif dt_init == "random":
    ...
    def A_log_init(d_state, d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            A_log = A_log[None].repeat(copies, 1, 1).contiguous()  # (copies, d_inner, d_state)的张量
            if merge:
    ...
    def D_init(d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            ...
            if merge:
                D = D.flatten(0, 1)
        D = nn.Parameter(D)  # Keep in fp32  将张量A_log封装为一个参数对象
...
###################################################前向传播
    ...
    def part_1(self, x: torch.Tensor):
    ...
    def part_2(self, x: torch.Tensor):
        x = self.layers[1](x)

        return x

    def part_3(self, x: torch.Tensor):
        x = self.layers[2](x)

        return x

    def part_4(self, x: torch.Tensor):
        x = self.layers[3](x)
        return x

    #def classifier_out1(self, x: torch.Tensor):
        #x = self.classifier(x)
        #return x
        
    def classifier_out1(self, x: torch.Tensor, extract_features=True):
        

        if extract_features:

            features = self.classifier[:4](x)
            
            #encoding_array.append(features.detach().cpu().numpy())
            #np.save('/home/admin1/DFMamba/dataset_tsne/DFM_nt_tsne.npy', encoding_array)
            
            output = self.classifier[4:](features)
    ...
    def classifier_out2(self, x: torch.Tensor):
        x = self.classifier(x)
        return x
